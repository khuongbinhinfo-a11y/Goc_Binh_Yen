...
if (!localPathArg) {
  console.error("Usage: node scripts/upload-audio-chut-tinh-xin-goi-cho-que.mjs <local-audio-path>");
  process.exit(2);
}

const localFile = resolve(process.cwd(), localPathArg);
if (!existsSync(localFile)) {
  console.error(`Audio file not found: ${localFile}`);
  process.exit(2);
}
...
const key = `${prefixAudio}/doc-tho/chut-tinh-xin-goi-cho-que.m4a`;
