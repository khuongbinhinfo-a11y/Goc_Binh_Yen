...
export default function UngHoPage() {
  ...
  const vietQrUrl = useMemo(() => {
    ...
    const addInfo = encodeURIComponent(transferNarrativeFull);
  ...
  }, [transferAccountName, transferAccountNumber, transferBank, transferNarrativeFull]);
  ...
  const handleCopyAccountNumber = async () => {
    ...
    try {
      ...
      window.setTimeout(() => setCopiedAccount(false), 2200);
    } catch {
  ...
  const handleCopyTransferContent = async () => {
    if (!transferNarrativeDisplay) {
      return;
    }

    try {
      await navigator.clipboard.writeText(transferNarrativeDisplay);
      setCopiedTransfer(true);
      window.setTimeout(() => setCopiedTransfer(false), 2200);
    } catch {
  ...
  return (
    <div className="min-h-screen bg-[#f3eadf] text-[#3d2a1f]">
      ...
      {showPopup ? (
        <div
          className="fixed inset-0 z-50 flex items-center justify-center bg-[#2b1b12]/65 px-4 py-6"
          ...
          >
            <div className="flex items-start justify-between gap-3">
              ...
              <button
                type="button"
                onClick={() => setShowPopup(false)}
                className="inline-flex h-8 w-8 items-center justify-center rounded-full border border-[#d3b393] text-[#6f4b35] transition hover:bg-[#f2dfcb]"
            ...
            <dl className="mt-4 space-y-2.5">
              <div className="flex items-start justify-between gap-3 rounded-xl bg-white/70 px-3 py-2.5">
                <dt className="text-xs font-semibold uppercase tracking-[0.12em] text-[#8b684f]">Số tài khoản</dt>
                <dd className="text-sm font-medium text-[#4a2f20]">{copy.accountNumber}</dd>
              </div>
              <div className="flex items-start justify-between gap-3 rounded-xl bg-white/70 px-3 py-2.5">
                <dt className="text-xs font-semibold uppercase tracking-[0.12em] text-[#8b684f]">Chủ tài khoản</dt>
                <dd className="text-sm font-medium text-[#4a2f20]">{transferAccountName}</dd>
              </div>
              <div className="flex items-start justify-between gap-3 rounded-xl bg-white/70 px-3 py-2.5">
                <dt className="text-xs font-semibold uppercase tracking-[0.12em] text-[#8b684f]">Ngân hàng</dt>
                <dd className="text-sm font-medium text-[#4a2f20]">{transferBank}</dd>
              </div>
            </dl>
              ...
              <div className="mt-3 space-y-3">
                ...
                <label className="block space-y-1.5 text-xs text-[#5f4332] sm:text-sm">
                  ...
                  <textarea
                    ...
                    placeholder="Bạn có thể để lại vài dòng nếu muốn"
                    rows={3}
                    className="w-full rounded-xl border border-[#d9bea4] bg-white px-3 py-2.5 text-sm text-[#3f2b20] outline-none ring-[#9f6b45] transition focus:ring"
                  />
                </label>
