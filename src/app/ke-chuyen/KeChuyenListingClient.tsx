...
const storyTagBySlug: Record<string, string> = {
  "ben-do-cu-qua-mot-mua-mua": "Sông nước",
  "dem-nghe-tieng-nuoc-chay": "Sông nước",
  "chuyen-nguoi-qua-cau-tre": "Cầu tre",
  "mui-khoi-bep-len-tu-xom-nho": "Quê nhà",
  "mot-buoi-cho-que-tan-muon": "Chợ quê",
  "anh-den-truoc-san-nha-dem-mat-dien": "Xóm làng",
  "nguoi-va-xuong-o-me-song": "Sông nước",
};

export default function KeChuyenListingClient() {
  const { locale } = useLocale();
  const copy = getReadingCopy(locale, "story").listing;
  const routePrefix = getContentRoutePrefix("story");
  const stories = getLocalizedContentList("story", locale);
  const taggedStories = useMemo(
    () =>
      stories.map((item) => ({
        ...item,
        tag: storyTagBySlug[item.slug] ?? item.category,
      })),
    [stories],
  );
  const featured = taggedStories.find((item) => item.isFeatured) ?? taggedStories[0];
  const listItems = taggedStories.filter((item) => item.slug !== featured.slug);
  const hasFallback = taggedStories.some((item) => item.i18nStatus.hasFallback);
