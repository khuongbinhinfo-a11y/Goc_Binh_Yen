...
type KeChuyenDetailClientProps = {
  story: LocalizedContentItem;
};

function AudioPlayerWithSuspense({
  audioUrl,
  queue,
  currentSlug,
  routePrefix,
...
export default function KeChuyenDetailClient({ story }: KeChuyenDetailClientProps) {
  ...
  const handleShareFacebook = async () => {
    const shareText = [story.title, "", story.content, "", getArticleUrl()].filter(Boolean).join("\n");
  ...
  const handleListenClick = () => {
    ...
    window.history.replaceState(null, "", `${routePrefix}/${story.slug}?autoplay=1#nghe-xem`);
  };
              ...
              <div className="mt-6 grid gap-5 md:grid-cols-2">
                {mediaItems.map((media) => (
                  <article key={media.title} className="soft-panel bg-[#fffaf5] p-6">
                    ...
                    {media.type === "audio" ? (
                      <Suspense fallback={<div className="mt-4 text-sm text-[#654939]">Đang tải trình phát...</div>}>
                        <AudioPlayerWithSuspense
                          audioUrl={media.href}
                          queue={audioQueue}
